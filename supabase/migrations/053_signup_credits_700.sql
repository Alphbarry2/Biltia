-- ============================================================
-- BILTIA — Migration 053 : solde d'inscription 300 → 700 crédits
-- ============================================================
-- POURQUOI (décision tarifaire user, 2026-07-14) :
--
-- Le prix d'une application sur mesure passe de 250 à 600 crédits (lib/plans.ts →
...
--
-- Conséquence directe sur l'essai gratuit : le plan Free promet « Créez votre
-- première application », et le hold est prélevé D'AVANCE. Avec 300 crédits offerts
-- et une app à 600, un nouvel inscrit se serait heurté à « crédits insuffisants »
-- AVANT d'avoir rien produit. La pire première impression possible, et elle aurait
-- été silencieuse : rien dans le code ne relie les deux constantes.
--
-- 700 = 1 application (600) + de quoi poser quelques questions ensuite.
--
...
alter table public.user_credits alter column balance set default 700;
...
begin
  ...
  -- NOUVEAU COMPTE STANDARD : 700 crédits (miroir de SIGNUP_FREE_CREDITS) + espace perso.
