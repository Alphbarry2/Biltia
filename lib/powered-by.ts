...
export function publicNotFoundPage(
  ...
  const t = esc(title);
  const m = esc(message);
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${t}</title><script src="https://cdn.tailwindcss.com"></script></head>
