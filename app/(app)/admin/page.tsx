...
function BarList({
  ...
  return (
    <div className="bg-white border border-[#ECECF2] rounded-2xl p-5 shadow-[0_4px_14px_rgba(60,40,120,0.08)]">
      <h3 className="text-sm font-bold text-[#0A0A0A] mb-4">{title}</h3>
      {rows.length === 0 ? (
        <p className="text-sm text-[#6E6E6C]">Aucune donnée pour l&apos;instant.</p>
      ) : (
        <div className="space-y-2.5">
          {rows.map((r) => (
            <div key={r.key} className="flex items-center gap-3">
              <span className="text-xs text-[#0A0A0A] w-40 truncate flex-shrink-0">
                {label(r.key)}
              </span>
              <div className="flex-1 h-2 bg-[#F6F6F9] rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-br from-indigo-500 via-violet-500 to-pink-500 rounded-full"
                  style={{ width: `${(r.count / max) * 100}%` }}
                />
              </div>
              <span className="text-xs font-semibold text-[#0A0A0A] tabular-nums w-8 text-right">
                {r.count}
              </span>
            </div>
          ))}
...
function Kpi({ icon, value, label }: { icon: React.ReactNode; value: number; label: string }) {
  return (
    <div className="bg-white border border-[#ECECF2] rounded-2xl p-5 shadow-[0_4px_14px_rgba(60,40,120,0.08)]">
      <div className="flex items-center gap-2 text-[#6E6E6C] mb-2">
        {icon}
...
export default function AdminPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full text-[#6E6E6C] text-sm">
  ...
  if (error || !data) {
    return (
      <div className="max-w-md mx-auto mt-20 bg-white border border-[#ECECF2] rounded-2xl p-8 text-center shadow-[0_8px_24px_rgba(60,40,120,0.12)]">
        <h2 className="text-lg font-bold text-[#0A0A0A] mb-2">Accès refusé</h2>
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 sm:px-8 py-8">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-xl font-bold text-[#0A0A0A]">Statistiques</h1>
      </div>
      ...
      {/* Activité 30 jours */}
      <div className="bg-white border border-[#ECECF2] rounded-2xl p-5 shadow-[0_4px_14px_rgba(60,40,120,0.08)] mb-6">
        <h3 className="text-sm font-bold text-[#0A0A0A] mb-4">
          Activité (30 derniers jours)
        </h3>
        {by_day.length === 0 ? (
          <p className="text-sm text-[#6E6E6C]">Aucune activité récente.</p>
        ) : (
          <div className="flex items-end gap-1 h-32">
            {by_day.map((d) => (
              <div key={d.day} className="flex-1 flex flex-col items-center justify-end" title={`${d.day} : ${d.count}`}>
                <div
                  className="w-full bg-[#7C3AED]/80 rounded-t"
          ...
          <div className="divide-y divide-border">
            {top_companies.map((c, i) => (
              <div key={i} className="flex items-center gap-3 py-2.5">
                <span className="text-xs text-[#6E6E6C] w-5 tabular-nums">{i + 1}</span>
                <span className="text-sm text-[#0A0A0A] flex-1 truncate">
                  {c.company || <span className="text-[#6E6E6C] italic">Sans entreprise</span>}
                </span>
                <span className="text-xs text-[#6E6E6C] w-44 truncate hidden sm:block">
                  {sectorLabel(c.sector)}
                </span>
                <span className="text-sm font-semibold text-[#0A0A0A] tabular-nums w-8 text-right">
                  {c.apps}
                </span>
              </div>
            ))}
