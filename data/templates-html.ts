...
load(); render();
  ...
  },
  {
    ...
    description: "Vue globale de vos indicateurs clés : chantiers, finances et équipes.",
    html: `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
...
${FONT_LINK}
<style>${CSS}
.section-title{font-size:13px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px}
</style>
</head>
<body>
<div class="app-header">
  <span class="app-title">📊 Tableau de bord</span>
  <span id="today-label" style="font-size:12px;color:#9CA3AF"></span>
</div>
<div class="app-main">
  <div style="padding:16px">
    <div class="section-title">Performance financière</div>
    <div class="kpi-grid" id="kpi-finance" style="padding:0;margin-bottom:16px"></div>

    <div class="section-title">Chantiers actifs</div>
    <div id="chantiers-list" style="margin-bottom:16px"></div>

    <div class="section-title">Alertes & Actions requises</div>
    <div id="alertes-list" style="margin-bottom:16px"></div>
...
<div class="tab-bar">
  <button class="tab-item active" id="tab-dash" onclick="showTab('dash')">📊<span>Synthèse</span></button>
  <button class="tab-item" id="tab-objectifs" onclick="showTab('objectifs')">🎯<span>Objectifs</span></button>
</div>

<div class="overlay" id="overlay-obj" style="display:none" onclick="closeObjModal()">
  <div class="modal" onclick="event.stopPropagation()">
    <div class="modal-title">Définir les objectifs</div>
    <div class="fg"><label class="fl">CA annuel objectif (€)</label><input id="obj-ca" type="number" placeholder="1500000"></div>
    <div class="fg"><label class="fl">Nombre chantiers cible</label><input id="obj-chantiers" type="number" placeholder="20"></div>
    <div class="fg"><label class="fl">Marge brute cible (%)</label><input id="obj-marge" type="number" placeholder="25"></div>
    <div class="fg"><label class="fl">Effectif prévu</label><input id="obj-effectif" type="number" placeholder="15"></div>
    <div class="modal-actions">
      <button class="btn btn-ghost" onclick="closeObjModal()">Annuler</button>
      <button class="btn btn-ink" onclick="saveObjectifs()">Enregistrer</button>
    </div>
  </div>
</div>
...
const DEFAULTS={
  chantiers:[
    {nom:'Résidence Les Pins',budget:280000,depense:142000,avancement:51,status:'En cours'},
    {nom:'École de Montbel',budget:42000,depense:0,avancement:0,status:'En attente'},
    {nom:'Extension Duval',budget:95000,depense:95000,avancement:100,status:'Terminé'}
  ],
  factures:[
    {montant:57600,status:'Payé',date:'2024-03-28'},{montant:28500,status:'Envoyé',date:'2024-04-05'},{montant:46200,status:'Accepté',date:'2024-04-12'}
  ],
  employes:[
    {nom:'Mohammed Amrani',heures:160},{nom:'Lucas Bertrand',heures:152},{nom:'Sofia Moreau',heures:176},{nom:'Ahmed Bouazza',heures:144}
  ],
  alertes:[
    {type:'stock',msg:'Ciment Portland : stock en dessous du seuil (18 sacs restants)'},
    {type:'facture',msg:'Facture FAC-2024-012 non payée — échéance dépassée de 5 jours'},
    {type:'chantier',msg:'Planning chantier École : démarrage dans 10 jours — ressources à confirmer'}
  ],
  objectifs:{ca:1500000,chantiers:20,marge:25,effectif:15}
};

function load(){
  try{ state=JSON.parse(localStorage.getItem(KEY)||'null')||null; }catch(e){ state=null; }
  if(!state){ state=DEFAULTS; save(); }
  if(!state.objectifs) state.objectifs=DEFAULTS.objectifs;
}
...
function renderDash(){
  document.getElementById('today-label').textContent=new Date().toLocaleDateString('fr-FR',{weekday:'short',day:'numeric',month:'short'});

  const totalCA=state.factures.reduce((s,f)=>s+(+f.montant||0),0);
  const paye=state.factures.filter(f=>f.status==='Payé').reduce((s,f)=>s+(+f.montant||0),0);
  const enCours=state.chantiers.filter(c=>c.status==='En cours').length;
  const hTotal=state.employes.reduce((s,e)=>s+(+e.heures||0),0);

  document.getElementById('kpi-finance').innerHTML=\`
    <div class="kpi"><div class="kpi-label">CA facturé</div><div class="kpi-value">\${(totalCA/1000).toFixed(0)}k€</div><div class="kpi-sub">ce mois</div></div>
    <div class="kpi"><div class="kpi-label">Encaissé</div><div class="kpi-value">\${(paye/1000).toFixed(0)}k€</div><div class="kpi-sub">\${totalCA?Math.round(paye/totalCA*100):0}% du facturé</div></div>
    <div class="kpi"><div class="kpi-label">Chantiers actifs</div><div class="kpi-value">\${enCours}</div></div>
    <div class="kpi"><div class="kpi-label">Heures équipe</div><div class="kpi-value">\${hTotal}h</div><div class="kpi-sub">\${state.employes.length} personnes</div></div>
  \`;

  document.getElementById('chantiers-list').innerHTML=state.chantiers.filter(c=>c.status==='En cours').map(c=>{
    const pct=Math.min(100,+c.avancement||0);
    const bgt=+c.budget||1;
    const dep=+c.depense||0;
    return \`<div class="card" style="margin-bottom:8px">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px">
        <div style="font-weight:700">\${c.nom}</div>
        <div style="font-size:12px;color:#6B7280">\${fmt(bgt)} €</div>
      </div>
      <div style="display:flex;gap:16px;margin-bottom:8px;font-size:12px">
        <span>Avancement : <b>\${pct}%</b></span>
        <span>Dépensé : <b>\${fmt(dep)} €</b></span>
        <span>Restant : <b>\${fmt(bgt-dep)} €</b></span>
      </div>
      <div class="prog-track"><div class="prog-fill" style="width:\${pct}%"></div></div>
    </div>\`;
  }).join('')||'<div class="empty" style="padding:20px">Aucun chantier en cours</div>';

  const alerteIcons={stock:'📦',facture:'💰',chantier:'🏗️'};
  document.getElementById('alertes-list').innerHTML=state.alertes.map(a=>\`
    <div style="display:flex;align-items:flex-start;gap:10px;padding:12px;background:#FEF3C7;border:1px solid #FDE68A;border-radius:12px;margin-bottom:8px">
      <span style="font-size:18px">\${alerteIcons[a.type]||'⚠️'}</span>
      <span style="font-size:13px;color:#92400E;font-weight:500">\${a.msg}</span>
    </div>
  \`).join('')||'<div style="color:#7C3AED;font-weight:600;padding:12px">✅ Aucune alerte en cours</div>';

  document.getElementById('activite-list').innerHTML=\`<div class="table-wrap"><table>
    <thead><tr><th>Date</th><th>Document</th><th>Montant</th><th>Statut</th></tr></thead>
    <tbody>\${[...state.factures].reverse().map(f=>\`
      <tr><td>\${f.date}</td><td>Facture</td><td style="font-weight:600">\${fmt(f.montant)} €</td>
      <td><span class="badge \${f.status==='Payé'?'badge-green':f.status==='Envoyé'?'badge-amber':'badge-gray'}">\${f.status}</span></td></tr>
    \`).join('')}</tbody>
  </table></div>\`;
}

function renderObjectifs(){
  const o=state.objectifs;
  const caActuel=state.factures.reduce((s,f)=>s+(+f.montant||0),0);
  const chantiersActuel=state.chantiers.length;
  const effectifActuel=state.employes.length;
  document.getElementById('kpi-finance').innerHTML='';
  document.getElementById('chantiers-list').innerHTML='';
  document.getElementById('activite-list').innerHTML='';
  document.getElementById('alertes-list').innerHTML='';

  let cont=document.getElementById('obj-container');
  if(!cont){ cont=document.createElement('div'); cont.id='obj-container'; document.querySelector('.app-main div').appendChild(cont); }
  cont.style.display='';

  function objRow(label,actuel,cible,unit){
    const pct=cible?Math.min(100,Math.round(actuel/cible*100)):0;
    return \`<div class="card" style="margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px">
        <div style="font-weight:700">\${label}</div>
        <div style="font-size:12px;color:#6B7280">\${actuel}\${unit} / \${cible}\${unit}</div>
      </div>
      <div class="prog-track"><div class="prog-fill" style="width:\${pct}%"></div></div>
      <div style="font-size:11px;color:#6B7280;margin-top:4px">\${pct}% de l'objectif atteint</div>
    </div>\`;
  }
  cont.innerHTML=\`
    <div style="margin-bottom:12px"><button class="btn btn-ghost" onclick="openObjModal()">✏️ Modifier les objectifs</button></div>
    \${objRow('CA annuel',fmt(caActuel),fmt(o.ca),'€')}
    \${objRow('Chantiers',chantiersActuel,o.chantiers,'')}
    \${objRow('Effectif',effectifActuel,o.effectif,' pers.')}
  \`;
}

function showTab(t){
  document.getElementById('tab-dash').className='tab-item'+(t==='dash'?' active':'');
  document.getElementById('tab-objectifs').className='tab-item'+(t==='objectifs'?' active':'');
  const oc=document.getElementById('obj-container');
  if(oc) oc.style.display=t==='objectifs'?'':'none';
  if(t==='dash') renderDash();
  else renderObjectifs();
}

function openObjModal(){
  const o=state.objectifs;
  document.getElementById('obj-ca').value=o.ca||'';
  document.getElementById('obj-chantiers').value=o.chantiers||'';
  document.getElementById('obj-marge').value=o.marge||'';
  document.getElementById('obj-effectif').value=o.effectif||'';
  document.getElementById('overlay-obj').style.display='flex';
}
function closeObjModal(){ document.getElementById('overlay-obj').style.display='none'; }

function saveObjectifs(){
  state.objectifs={
    ca:+document.getElementById('obj-ca').value||0,
    chantiers:+document.getElementById('obj-chantiers').value||0,
    marge:+document.getElementById('obj-marge').value||0,
    effectif:+document.getElementById('obj-effectif').value||0
  };
  save(); closeObjModal(); renderObjectifs();
}
...
export const TEMPLATE_APPS: TemplateApp[] = BASE_APPS.map(
  (a) => PREMIUM_APPS.find((p) => p.id === a.id) ?? a
);
