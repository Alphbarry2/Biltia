...
import {
  earliestBookableIso, isoOf, slotsForDate, TZ_LABEL,
  HEADCOUNT_OPTIONS, LOOKING_FOR_OPTIONS, formatSlotFr, type Option,
} from "@/lib/demo-booking";

const WEEK_LABELS = ["L", "M", "M", "J", "V", "S", "D"];

function monthLabel(y: number, m: number): string {
  const s = new Intl.DateTimeFormat("fr-FR", { month: "long", year: "numeric", timeZone: "UTC" })
...
export function SlotPicker({
  ...
  return (
    <div>
      {/* Calendrier */}
      <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-[#9A9AA6]">1. Quel jour&nbsp;?</p>
      <div className="rounded-2xl border border-[#ECECF2] bg-white p-3 sm:p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-[14px] font-bold text-[#0A0A0A]">{monthLabel(view.y, view.m)}</span>
          <button
            type="button"
            onClick={() => shift(1)}
      ...
      {date && (
        <div className="mt-5">
          <p className="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#9A9AA6]">
            2. Quelle heure&nbsp;? <span className="font-medium normal-case tracking-normal text-[#B4ADC4]">({TZ_LABEL})</span>
          </p>
...
const STEPS: StepDef[] = [
  { key: "company", title: "Quelle est votre entreprise ?", subtitle: "Le nom de votre société." },
  { key: "website", title: "Votre site web ?", subtitle: "Optionnel — pour mieux vous connaître." },
  { key: "headcount", title: "Vous êtes combien ?", subtitle: "L'effectif de votre entreprise." },
  { key: "looking_for", title: "Que recherchez-vous ?", subtitle: "Ce qui vous amène chez Biltia." },
  { key: "message", title: "Un mot sur votre besoin ?", subtitle: "Optionnel — quelques lignes suffisent." },
  { key: "contact", title: "Vos coordonnées", subtitle: "Pour vous envoyer la confirmation." },
];
...
export default function DemoBookingModal({ onClose }: { onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrMsg(json.error || "Une erreur est survenue. Réessayez.");
        setPhase("error");
        return;
      }
      setPhase("done");
    } catch {
      setErrMsg("Réseau indisponible. Réessayez.");
      setPhase("error");
    }
  };
  ...
  const body = (
    <div className="fixed inset-0 z-[200] flex items-end justify-center p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Réserver une démo">
      ...
      <div className="relative flex max-h-[94vh] w-full max-w-[560px] flex-col overflow-hidden rounded-t-[26px] bg-white shadow-[0_40px_120px_rgba(20,20,50,0.4)] sm:rounded-[26px]">
        ...
        <div className="flex items-center justify-between gap-3 px-5 pt-4 pb-3 sm:px-6">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-[16px] font-bold tracking-[-0.01em] text-[#0A0A0A]">Réserver une démo</h2>
          </div>
          <button onClick={onClose} aria-label="Fermer" className="grid h-8 w-8 place-items-center rounded-full text-[#6E6E7A] transition-colors hover:bg-black/[0.05]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5 sm:px-6">
          ...
          {phase === "form" && (
            <div className="pt-1">
              ...
              <p className="text-[11px] font-semibold uppercase tracking-wider text-[#9A9AA6]">Étape {step + 1} sur {STEPS.length}</p>
              ...
              {STEPS[step].key === "company" && (
                <TextField icon={Building2} value={f.company_name} onChange={(e) => set("company_name")(e.target.value)} placeholder="Nom de l'entreprise" autoFocus maxLength={120} />
              )}
              {STEPS[step].key === "website" && (
                <TextField icon={Globe} value={f.website} onChange={(e) => set("website")(e.target.value)} placeholder="https://votre-site.com" maxLength={200} inputMode="url" />
              )}
              {STEPS[step].key === "headcount" && (
                <OptionGrid options={HEADCOUNT_OPTIONS} value={f.headcount} onChange={set("headcount")} />
              )}
              {STEPS[step].key === "looking_for" && (
                <OptionGrid options={LOOKING_FOR_OPTIONS} value={f.looking_for} onChange={set("looking_for")} />
              )}
              {STEPS[step].key === "message" && (
                <div className="flex items-start gap-2.5 rounded-2xl border border-[#ECECF2] bg-white px-4 py-3 focus-within:border-[#7C3AED] focus-within:ring-2 focus-within:ring-[#7C3AED]/20">
                  ...
                  <textarea value={f.message} onChange={(e) => set("message")(e.target.value)} placeholder="Ex : je veux automatiser mes relances de devis…" rows={4} maxLength={1000} className="w-full resize-none bg-transparent text-[14px] text-[#0A0A0A] outline-none placeholder:text-[#B4ADC4]" />
                </div>
              )}
              {STEPS[step].key === "contact" && (
                <div className="grid gap-2.5">
                  <TextField icon={User} value={f.contact_name} onChange={(e) => set("contact_name")(e.target.value)} placeholder="Votre nom" autoFocus maxLength={120} />
                  <TextField icon={Mail} value={f.contact_email} onChange={(e) => set("contact_email")(e.target.value)} placeholder="Email professionnel" type="email" maxLength={160} />
                  <TextField icon={Phone} value={f.contact_phone} onChange={(e) => set("contact_phone")(e.target.value)} placeholder="Téléphone (optionnel)" type="tel" maxLength={40} />
                </div>
              )}

              {date && time && (
                <p className="mt-4 flex items-center gap-1.5 rounded-xl bg-[#F5F2FD] px-3 py-2 text-[12.5px] font-medium text-[#6D4AE0]">
                  <CalendarDays className="h-3.5 w-3.5" /> {formatSlotFr(date, time)}
                </p>
              )}
            </div>
          )}
          ...
          {phase === "sending" && (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
              ...
              <p className="text-[14px] text-[#5B5B66]">Envoi de votre demande…</p>
            </div>
          )}
          ...
          {phase === "done" && (
            <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
              ...
              <h3 className="text-[20px] font-black tracking-[-0.02em] text-[#0A0A0A]">Demande envoyée&nbsp;!</h3>
              ...
              {date && time && (
                <p className="flex items-center gap-1.5 rounded-xl bg-[#F5F2FD] px-3.5 py-2 text-[13px] font-semibold text-[#6D4AE0]">
                  <CalendarDays className="h-4 w-4" /> {formatSlotFr(date, time)}
                </p>
              )}
            </div>
          )}
        ...
        {(phase === "slot" || phase === "form") && (
          <div className="flex items-center gap-3 border-t border-[#ECECF2] px-5 py-4 sm:px-6">
            {phase === "form" && (
              <button
                type="button"
            ...
            >
              {phase === "slot" ? "Continuer" : isLast ? "Confirmer" : "Suivant"} <ArrowRight className="h-4 w-4" />
            </button>
          </div>
        )}
      </div>
